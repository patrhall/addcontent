...
{
    /// <summary>
    /// Summary description for EditNewsLetter.
    /// </summary>
    public partial class EditText : AIM.Base.Modules.Page.AimEdit
    {
        private int oID;
        ...
        private void setLanguage()
        {
            ...
            lblPreview.Text = Language.getValue("lblPreview");

            imgPreview.Attributes.Add("style", "vertical-align:middle;");
        }

        private void initPreview()
        {
            if (Convert.ToBoolean(Preference.getValue("content", "previewpage", "used")))
            {
                pnlPreview.Visible = true;

                string kID = (from o in db.Objects where o.ObjectID == oID select o.ObjectKey).First<string>();

                rwPreview.Width = Unit.Pixel(Convert.ToInt32(Preference.getValue("content", "previewpage", "width")));
                rwPreview.Height = Unit.Pixel(Convert.ToInt32(Preference.getValue("content", "previewpage", "height")));

                string url = Preference.getValue("content", "previewpage", "domain");

                switch (Preference.getValue("content", "directurl", "urlrewrite"))
                {                    
                    case "NOMASTER":
                    case "MASTER":
                        //url += "/" + oID.ToString() + "/" + Preference.getValue("content", "previewpage", "page") + "?kID=" + kID;
                        //break;                                           
                    case "NOTUSED":
                    default:
                        url += "/" + Preference.getValue("content", "previewpage", "page") + "?oID=" + oID.ToString() + "&kID=" + kID;
                        break;
                }

                rwPreview.NavigateUrl = url;
            }
            else
